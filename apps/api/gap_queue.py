...
Backed by the corpus Postgres (NOESIS_CORPUS_DSN).
"""
            ...
            async with conn.transaction():
                ...
                row = await conn.fetchrow(
                    """UPDATE noesis_corpus_gap_queue q SET status='running', updated_at=now()
                       ...
                       WHERE q.id=c.id
                       RETURNING q.id, q.connector, q.query, q.lim, q.tenant_id""",
                    self._vertical)
        if row is None:
            return None
