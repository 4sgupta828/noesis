...
- **W8 — Frontend architecture (NEW, its own design sub-project)**: how a vertical **declares its UI** (entity views, deliverable renderers) and how much of `web/research_system` is a generic shell vs. per-vertical components. Panel: this "rivals the kernel in complexity" — do a design pass before P4.
  **UI principles (locked by user):** (1) **Responsive & mobile-friendly from day one** — mobile is a first-class layout, not an afterthought; design mobile-up. (2) **Minimal & intuitive** — sparse, calm, low-chrome; the research content is the interface. (3) **NOT a copy of, and not influenced by, the current `web/research_system` UI** — this is a clean-sheet design; do not port its components, layout, or visual language. (4) **UI may be vertical-specific** — the kernel/app provides a neutral shell + primitives, but a vertical MAY declare its own views/renderers (regulatory tables vs. financial fact-coordinate cards vs. legislative diffs); the "generic shell vs. per-vertical component" boundary is the core W8 design question. Load the `frontend-design` skill at W8 start for aesthetic direction.
...
1. Repo/CI (done: scaffold + invariant gate). Add the **AST import-graph check** (W5), the `VerticalConformance` runner (W2), and **the Provider Port + cassette layer (W10) with `replay` as the default + a CI guard that fails any `live` call** — this is what makes every subsequent phase's tests and evals cost zero credits.
   **Contract-hardening (done, per the flexibility review):** the `VerticalManifest` slots are **typed Protocols** (`contract/protocols.py` — Connector, FetchStrategy, Parser, RetrievalSource, GatingPolicy, CitationVerifier, Persona, EntityView, **UIContract**), backed by **domain-neutral core DTOs** (`contract/dto.py` — `EntityRef`/`DocumentRef`/`BlockHit`/`Locator`/`Facets`, no domain columns). Conformance checks are **behavioral** (`isinstance` against the runtime-checkable Protocols), the invariant grep is broadened (jurisdiction/case_number/doc_family/filing/rate_base/roe), and CI no longer masks test/lint failures. This closes the "Any placeholders let legacy re-import" risk **before** any porting begins.
...
3. **Re-author the qa scorer** vertical-parameterized: keep generic correctness/citation-grounding; **remove** `LEGAL_SOURCE_FAMILIES`-in-core + corpus-vs-ingest recall + `state_bleed`; **retire/migrate the OH-cutover gold cases** (their verdicts are defined by the removed gates — they cannot and should not be "reproduced"). **[DONE]** `eval/qa_scoring.py` — generic gates (value-present w/ standalone-number guard, forbidden-values, phrases, refused-correctly, answered, citation-grounded with a **generic** per-case citation-constraint replacing the legal-family gate); domain-neutral fixtures.
4. Port `lookup` scorer unchanged. **[DONE]** `eval/lookup_scoring.py`.

**P0 code status: COMPLETE** (repo/CI/guardrails, provider+cassette layer, typed contract+DTOs+UI, conformance runner, lookup + qa scorers — 20 offline tests green). The only remaining P0 item is the **one-time budgeted baseline capture** (recording cassettes against current prod), deferred until credits are intentionally spent.
