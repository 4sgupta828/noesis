"""Specialist-Panel DDx differential-synthesis paired A/B harness — the PANEL synthesis WITHOUT vs
WITH the differential-first addendum (NOESIS_PANEL_DIFFERENTIAL) on diagnostic cases.

Modeled EXACTLY on ab_ddx.py (same flipped-pairwise LLM judge, DDx structural gold metrics,
consistent_winner, sign_test, subprocess-per-arm run.py invocation). Only the arm envs and the
output filename change: both arms drive the PANEL (NOESIS_EVAL_PANEL=1) instead of the reasoned
engine, and the A/B knob is NOESIS_PANEL_DIFFERENTIAL (the panel's differential-first synthesis
addendum) rather than NOESIS_DIFFERENTIAL_FORMAT.
...
NOESIS_* knobs the arm explicitly sets). BOTH arms set NOESIS_EVAL_PANEL=1 so run.py drives
`svc.plan_panel(...)` → `svc.ask_panel(...)` — the specialist panel — with the shared-contract and
dedup upgrades on (NOESIS_PANEL_CONTRACT=1, NOESIS_PANEL_DEDUP=1), and BOTH set
NOESIS_PANEL_DIFFERENTIAL explicitly (A="" so neither inherits a prod value):

  A (decision grid): NOESIS_EVIDENCE_IDENTITY=1 NOESIS_CLAIM_CONGRUENCE=1 NOESIS_EVAL_PANEL=1
                NOESIS_PANEL_CONTRACT=1 NOESIS_PANEL_DEDUP=1 NOESIS_PANEL_DIFFERENTIAL=""
  B (differential synthesis): same + NOESIS_PANEL_DIFFERENTIAL=1
...
Usage:
  .venv/bin/python evals/realworld/ab_panel_ddx.py \
...
# Both arms convene the specialist PANEL (NOESIS_EVAL_PANEL=1) with the shared-contract + dedup
# upgrades on; both set NOESIS_PANEL_DIFFERENTIAL explicitly so neither inherits a prod value.
# A="" (decision grid), B="1" (differential-first synthesis).
ARM_A_ENV = {"NOESIS_EVIDENCE_IDENTITY": "1", "NOESIS_CLAIM_CONGRUENCE": "1",
             "NOESIS_EVAL_PANEL": "1", "NOESIS_PANEL_CONTRACT": "1", "NOESIS_PANEL_DEDUP": "1",
             "NOESIS_PANEL_DIFFERENTIAL": ""}
ARM_B_ENV = {**ARM_A_ENV, "NOESIS_PANEL_DIFFERENTIAL": "1"}
...
def main() -> None:
    ...
    dest = RUNS / f"ab-panel-ddx-{stamp}.json"
