...
@check("gating-policy-declared", "P2")
def _gating(m: VerticalManifest) -> tuple[bool, str]:
    return (m.gating_policy is not None, "present" if m.gating_policy else "missing")


@check("persona-and-authority-declared", "P3")
def _persona(m: VerticalManifest) -> tuple[bool, str]:
    ok = m.persona is not None and m.authority_policy is not None
    return (ok, "present" if ok else "persona/authority missing")
