...
Spec: learnings/knowledgegraph.md (v2 + A9). Typed edges between `noesis_topic` labels,
...
_DDL = """
CREATE TABLE IF NOT EXISTS noesis_topic_edge (
...
CREATE INDEX IF NOT EXISTS noesis_edge_evidence_doc ON noesis_edge_evidence (document_id);
"""
...
class GraphStore:
    ...
    async def add_evidence(self, *, edge_id: str, document_id: str, quote: str = "",
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO noesis_edge_evidence (edge_id, document_id, quote, session_id, label)
                   VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING""",
                edge_id, document_id, quote[:1000], session_id, label)
            row = await conn.fetchrow(
                """UPDATE noesis_topic_edge SET seen_count =
                     (SELECT count(DISTINCT document_id) FROM noesis_edge_evidence WHERE edge_id=$1),
                     updated_at=now()
    ...
    async def set_status(self, edge_id: str, status: str) -> bool:
        ...
        async with pool.acquire() as conn:
            res = await conn.execute(
                "UPDATE noesis_topic_edge SET status=$2, needs_review=false, updated_at=now() "
    ...
    async def invalidate_from_events(self, events: list[dict]) -> dict:
        ...
        async with pool.acquire() as conn:
            if dead:
                res = await conn.execute(
                    """UPDATE noesis_topic_edge e SET status='demoted', updated_at=now()
                       WHERE e.status != 'demoted' AND e.provenance != 'curated'
                         AND EXISTS (SELECT 1 FROM noesis_edge_evidence v
                                     WHERE v.edge_id=e.id AND v.document_id = ANY($1::text[]))
                         AND NOT EXISTS (SELECT 1 FROM noesis_edge_evidence v
                                         WHERE v.edge_id=e.id
                                           AND NOT (v.document_id = ANY($1::text[])))""", dead)
                demoted = int((res or "UPDATE 0").split()[-1])
            if stale:
                res = await conn.execute(
                    """UPDATE noesis_topic_edge e SET needs_review=true, updated_at=now()
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                """SELECT document_id, quote, session_id, label, created_at
                   FROM noesis_edge_evidence WHERE edge_id=$1 ORDER BY created_at""", edge_id)
    ...
    async def stats(self) -> dict:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT status, count(*) AS n FROM noesis_topic_edge GROUP BY status")
            ev = await conn.fetchrow("SELECT count(*) AS n FROM noesis_edge_evidence")
