...
def test_every_case_is_well_formed():
    for cid, c in CLINICAL_GOLD.items():
        ...
        assert c.get("expect") in ("value", "refuse", "absence", "rank"), f"{cid}: bad expect"
...
def test_refuse_absence_cases_have_no_answer_gold():
    # a refuse/absence case must not also demand positive answer content (contradictory gold): the
    # correct behavior is to state the absence, not produce a specific value/floor.
    for cid, c in CLINICAL_GOLD.items():
        if c.get("expect") in ("refuse", "absence"):
            assert not c.get("required_phrases"), f"{cid}: refuse/absence case must not require phrases"
            assert not c.get("expected_values"), f"{cid}: refuse/absence case must not expect values"
            assert not c.get("evidence_floor_kinds"), f"{cid}: refuse/absence case has no evidence floor"


def test_coverage_is_stratified():
    cats = {c.get("category") for c in CLINICAL_GOLD.values()}
    assert {"treatment", "safety", "absence"} <= cats, f"missing categories: {cats}"
    # the risk-weighting + critical gate only bite if high-risk + absence cases exist
    assert any(c.get("clinical_risk") == "high" for c in CLINICAL_GOLD.values()), "no high-risk case"
    assert sum(1 for c in CLINICAL_GOLD.values() if c.get("expect") == "absence") >= 2, "too few absence cases"
