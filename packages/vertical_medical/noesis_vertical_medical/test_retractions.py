...
def _fake_fetch(retracted_ids):
    calls = []

    async def fetch(url, params):
        calls.append(params["query"])
        hits = [{"id": i} for i in retracted_ids if f"EXT_ID:{i}" in params["query"]]
...
def test_retraction_lineage_maps_back_to_document_ids():
    fetch = _fake_fetch({"9500320"})
