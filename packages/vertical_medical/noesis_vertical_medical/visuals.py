...
- The EDGE/relationship is the danger zone: only draw an arrow between two steps if the ANSWER ITSELF
  states that connection, and put the supporting span in the edge's `quote`. Never infer a link the
  answer didn't make (no "A probably causes B", no textbook knowledge the answer didn't state).
