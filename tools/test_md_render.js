...
t('question: paragraphs + list kept, no bold, long class', /qlong/.test(h) && (h.match(/<li>/g)||[]).length===2 && !/\*\*|<strong>/.test(h) && (h.match(/<p\b/g)||[]).length===3);
...
h=mdToHtml('## Ranked differential\n\n**1. Unstable angina (UA) / evolving NSTE-ACS — most likely given the clinical trajectory**\nThe patient has a 3-week crescendo pattern [1].');
t('long bold entry line → entry sub-heading, and NO lede style', /mdh-entry/.test(h) && !/mdlede/.test(h));
h=mdToHtml('## Bottom line\nStop it now [1].\n\n## Assessment\n'+'x '.repeat(220));
