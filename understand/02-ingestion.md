...
- **Europe PMC full-text is opt-in** via `NOESIS_EPMC_FULLTEXT` (default off,
...
The flow (all flag-gated behind `NOESIS_GAP_HEALING`, default off):
...
> `noesis_corpus_gap_queue` (ingest jobs) all share `NOESIS_CORPUS_DSN`. Each
