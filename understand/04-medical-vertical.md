...
- **`MEDICAL_CLINICAL_SYNTHESIS_FORMAT`** (`answer_format.py:69-123`) — an A/B
  variant behind `NOESIS_CLINICAL_SYNTHESIS` (default off). It uses the **same
...
Both are gated behind `NOESIS_STRUCTURED_ANSWERS`; when off, the kernel's plain
...
Both are opt-in: gap behind `NOESIS_GAP_HEALING`, suggest behind
`NOESIS_CONVERSATION` (`app.py:218-219`).
