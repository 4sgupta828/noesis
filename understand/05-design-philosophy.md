...
inventory in [the API layer](README.md): `NOESIS_STRUCTURED_ANSWERS`,
`NOESIS_CLINICAL_SYNTHESIS`, `NOESIS_VISION`, `NOESIS_GAP_HEALING`,
`NOESIS_STREAM`, `NOESIS_COUNTRY_SCOPE`, `NOESIS_CONVERSATION`,
`NOESIS_CLAIMS_FIRST`, `NOESIS_EVIDENCE_SELECT` (all in `app.py:29-93`, `225-230`).
...
attempted under pytest without an explicit `NOESIS_ALLOW_LIVE=1` opt-in
