...
## Part A — Evidence-fitness v1 (flag `NOESIS_EVIDENCE_FITNESS`, default OFF)
...
In `_rank_claims_by_relevance` (`react.py:279`), when `NOESIS_EVIDENCE_FITNESS` is on, combine the
...
2. **Part A (evidence-fitness v1)** → flip `NOESIS_EVIDENCE_FITNESS` ON in the eval.
